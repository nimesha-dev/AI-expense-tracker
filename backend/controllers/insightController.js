...
export const buildMonthlyInsight = async (userId) => {
    ...
    breakdown AS (
        SELECT
            c.name AS category, SUM(t.amount) AS amount
        FROM transactions t
    ...
    trend AS (
        ...
        SELECT
            ...
            (SELECT json_agg(json_build_object('category', category, 'amount', amount)) FROM breakdown) AS breakdown,
            (SELECT json_agg(json_build_object('month', month, 'income', income, 'expenses', expenses)) FROM trend) AS trend
        `,
        [userId]
    );
